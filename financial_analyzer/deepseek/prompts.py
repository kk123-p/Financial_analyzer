...
ANALYST_ROLES = {
    ...
    "risk": {
        ...
        "system_prompt": (
            ...
            "分析时必须基于提供的数据，严禁臆测。"
            "\n\n【思维框架】分析前请先列出你将检查的以下关键维度："
            "\n1. 杠杆与偿债能力（资产负债率、有息负债/EBITDA、利息保障倍数、股权质押比例）"
            "\n2. 现金流脆弱性（经营现金流与净利润背离、应收账款异常增长、存货积压、短期有息负债覆盖倍数）"
            "\n3. 反身性风险（股价下跌→融资困难→基本面恶化的传导路径、质押平仓线距离）"
            "\n4. 治理与透明度（关联交易占比、审计意见、管理层持股变动、信息披露质量）"
            "\n5. 尾部风险暴露（极端情景下的最大回撤、商誉减值风险、诉讼/合规风险敞口）"
            "\n\n【引用规范】引用数据时必须标注指标名、数值、报告期，"
            ...
            "\n\n【工具使用指引】可使用 get_financial_metric 查询精确财务数据，"
            "使用 get_historical_trend 查看关键指标的历史趋势，"
            "使用 get_anomaly_signals 查询异常信号检测结果。"
...
语言一致性规则：
- 全部使用中文输出，包括数字说明和表格标题
- 专业术语首次出现时附带中文解释，如「EBITDA（息税折旧摊销前利润）」
- 不得中英文混杂，英文缩写除外

引用执行规则：
- 每个数据引用必须包含：指标名称、具体数值、报告期
- 格式示例：「ROE = 15.3%（2024年报）」
- 推断性判断必须标注 [推断]，事实性数据标注 [事实]
- 无数据支撑的论点不得作为最终结论

结构化输出格式：
- 每轮辩论使用统一的 Markdown 标题层级（## 轮次标题，### 分析师标题）
- 关键结论使用 **加粗** 标注
- 数据表格使用标准 Markdown 表格格式
- 风险提示使用 > 引用块格式
"""
...
CROSS_EXAM_DIRECTIONS = {
    "value": (
        "- 重点质疑成长分析师给予的成长溢价是否超出安全边际\n"
        "- 检验风控师的风险评估是否忽略了资产价值底线\n"
        "- 用清算价值和历史估值分位作为反驳依据"
    ),
    "growth": (
        "- 重点质疑价值分析师的静态估值是否忽略了成长性\n"
        "- 检验风控师是否过度悲观而忽略了市场扩张机会\n"
        "- 用行业增速、渗透率和竞争壁垒数据作为反驳依据"
    ),
    "risk": (
        "- 重点质疑价值分析师的安全边际计算是否忽略了隐性负债\n"
        "- 检验成长分析师的增长假设是否考虑了反身性风险\n"
        "- 用杠杆数据、现金流异常信号和治理风险作为反驳依据"
    ),
}


def build_debate_round2(round1_statements, role_key: str = "") -> str:
    """构建辩论第二轮提示词

    Args:
        round1_statements: dict {analyst_id: statement_text} 或 str
        role_key: 分析师角色key (value/growth/risk)，为空则返回通用提示词
    """
    ...
    if role_key:
        role = ANALYST_ROLES.get(role_key, ANALYST_ROLES["value"])
        cross_direction = CROSS_EXAM_DIRECTIONS.get(role_key, CROSS_EXAM_DIRECTIONS["value"])
        return DEBATE_ROUND2_PROMPT.format(
            role_name=role["name"],
            system_prompt=role["system_prompt"],
            all_analyses=all_analyses,
            质询方向=cross_direction,
        )
    else:
        return (
            f"## 第二轮：交叉质询\n\n"
...
CITATION_VERIFICATION_PROMPT = """## 引用数据验证

你是投研系统的数据验证员。请检查以下辩论内容中的数据引用是否准确。

### 辩论内容
{debate_text}

### 验证要求
请逐条检查所有数据引用，对每条引用给出验证结果：
...
对每条引用输出一行：
- [准确] 指标名 = 数值（报告期）
- [不准确] 指标名 = 引用数值（报告期）→ 实际应为 X，问题：...
- [无法验证] 指标名 = 数值（报告期），原因：数据未提供
...
def build_citation_verification_prompt(debate_text: str) -> str:
    """构建引用验证提示词

    Args:
        debate_text: 前两轮辩论的完整文本
    """
    return CITATION_VERIFICATION_PROMPT.format(debate_text=debate_text)
